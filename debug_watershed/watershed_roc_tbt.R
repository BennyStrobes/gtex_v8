...
roc_analysis <- function(data_input, number_of_dimensions, phi_init, lambda_costs, lambda_pair_costs, pseudoc, inference_method, independent_variables, theta_pair_init, lambda_pair_inity, seed_number, lambda_inity) {
	...
	#gam_data <- genomic_annotation_model_cv(feat_train, binary_outliers_train, nfolds, lambda_costs, lambda_pair_costs, independent_variables, inference_method, gradient_descent_threshold, theta_pair_init)
	 gam_data <- logistic_regression_genomic_annotation_model_cv(feat_train, binary_outliers_train, nfolds, lambda_costs, lambda_pair_costs, independent_variables, inference_method, theta_pair_init, seed_number, lambda_inity)
...
pr_curve_aggregrated_across_tissues <- function(roc_vi, roc_independent, output_file) {
	options(bitmapType = 'cairo', device = 'pdf')
	precision <- c()
	recall <- c()
	prediction_type <- c()
	...
	precision <- c(precision, roc_vi$watershed_precision)
	recall <- c(recall, roc_vi$watershed_recall)
	prediction_type <- c(prediction_type, rep("watershed_tbt", length(roc_vi$watershed_precision)))

	precision <- c(precision, roc_independent$watershed_precision)
	recall <- c(recall, roc_independent$watershed_recall)
	prediction_type <- c(prediction_type, rep("river_tbt", length(roc_independent$watershed_precision)))

	precision <- c(precision, roc_vi$median_precision)
	recall <- c(recall, roc_vi$median_recall)
	prediction_type <- c(prediction_type, rep("river_median", length(roc_vi$median_precision)))

	df <- data.frame(precision, recall, prediction_type=factor(prediction_type, levels=c("watershed_tbt","river_tbt", "river_median")))
...
theta_pair_init <- as.numeric(args[6])
lambda_pair_inity <- as.numeric(args[7])
lambda_inity <- as.numeric(args[8])
...
roc_object_vi <- roc_analysis(data_input, number_of_dimensions, phi_init, lambda_costs, lambda_pair_costs, pseudoc, inference_method, independent_variables, theta_pair_init, lambda_pair_inity, seed_number, lambda_inity)
saveRDS(roc_object_vi, paste0(output_root, "_roc_object.rds"))
#roc_object_vi <- readRDS(paste0(output_root, "_roc_object2.rds"))
...
output_root <- paste0(output_stem,"_inference_", inference_method, "_independent_", independent_variables)
roc_object_independent <- roc_analysis(data_input, number_of_dimensions, phi_init, lambda_costs, lambda_pair_costs, pseudoc, inference_method, independent_variables, theta_pair_init, lambda_pair_inity, seed_number, lambda_inity)
...
for (tissue_num in 1:(number_of_dimensions+1)) {
	...
	median_auc <- tissue_roc_vi$median_pr_auc
	print(paste0(tissue_name, ": ", auc_vi, " ", auc_independent, " ", median_auc))
...
#######################################
## Visualize precision-recall curves for river, watershed and MEDIAN RIVER (aggregated across tissues)
#######################################
output_file <- paste0(output_stem, "aggregated_across_tissues_pr_curve.pdf")
pr_curve_aggregrated_across_tissues(roc_object_vi$roc[[50]]$evaROC, roc_object_independent$roc[[50]]$evaROC, output_file)
